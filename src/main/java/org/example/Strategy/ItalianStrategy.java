 ...
 * Class with applied moves roles from Italian version of the game.
 * In this version: normal pawn can not beat back, King can not move multiple fields,
 * normal pawn can not beat up a king.
 */
public class ItalianStrategy implements MovesStrategy {
    int nrOfFields;
    ...
    public ItalianStrategy(int nrOfFields) {
    ...
    public void addlegalJumps(final ArrayList<Movement> moves, final int distance, final Pawn[][] pawnsBoard, final Player player, final int row, final int col) {
        ...
        ItalianStrategy.Function function = new ItalianStrategy.Function() {
            @Override
