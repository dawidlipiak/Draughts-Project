...
public class LegalMoves {
    ...
    private boolean legalJump(Player player, Pawn [][] pawnsBoard, int fromRow, int fromCol, int betweenRow, int betweenCol, int toRow, int toCol) {
        ...
        if (player.isFirstPlayer()) {
            if (pawnsBoard[fromRow][fromCol].getColor() == player.getColor() && pawnsBoard[fromCol][fromCol].getState() == PawnState.NORMAL && toRow > fromRow) {
    ...
    public void addlegalMoves(ArrayList<Movement> moves, int distance, Pawn [][] pawnsBoard, Player player, int row, int col){
        if (pawnsBoard[row][col].getColor() == player.getColor()) {
            if (legalJump(player,pawnsBoard, row, col, row+1, col+1, row+distance, col+distance)) {
                moves.add(new Movement(row, col, row + distance, col + distance));
            }
            if (legalJump(player,pawnsBoard, row, col, row-1, col+1, row-distance, col+distance)) {
                moves.add(new Movement(row, col, row - distance, col + distance));
            }
