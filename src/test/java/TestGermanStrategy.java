...
public class TestGermanStrategy {
    ...
    /**
     * Set up board with no pawns, number of fields, player with Black pawns and version of the game.
     */
    @Before
    public void setUp() {
        nrOfFields = 8;
        strategy = new GermanStrategy(nrOfFields);
    ...
    public void legalMoveTestWithOneKing() {
        pawnsBoard[0][0].setState(PawnState.QUEEN);
        pawnsBoard[0][0].setColor(Color.BLACK);
        pawnsBoard[2][2].setState(PawnState.NORMAL);
        pawnsBoard[2][2].setColor(Color.WHITE);
    ...
    public void legalMoveTestForMovesBack2() {
        pawnsBoard[1][1].setState(PawnState.QUEEN);
        pawnsBoard[1][1].setColor(Color.BLACK);
        pawnsBoard[2][0].setState(PawnState.NORMAL);
        pawnsBoard[2][0].setColor(Color.WHITE);
        pawnsBoard[2][2].setState(PawnState.NORMAL);
    ...
    public void legalMoveTestBeat() {
        ...
        legalMoves = strategy.getLegalMoves(player, pawnsBoard);
        // In German version we can beat back, so we only have 1 move.
        Assert.assertEquals(legalMoves.length, 1);
    }
